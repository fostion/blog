...
var MusicSearchModule = require('./native_module/MusicSearchModule');
//引入自定义组件
...
//    }
  ...
  notifyUpdate(data){
     musicList.notifyDataChane(data);
  }

    render(){
          console.log("render");
          if(this.state.isFirstLoad){
            this.search_local_music();
    ...
    renderLoading(){
          return (
            <View style={ styles.container }>
                <MusicHeader search_local_music = { this.search_local_music.bind(this) } ></MusicHeader>
                <Text>
                加载中...
               </Text>
          </View>
    ...
    renderContent(){
          ...
          <View style={ styles.container }>
              <MusicHeader search_local_music = { this.search_local_music.bind(this) } ></MusicHeader>
              <MusicItem
               ref={(ref: MusicItem) => { musicList = ref }}
               data={ musicBeanData } onItemClick = { this.onItemClick.bind(this) } ></MusicItem>
              <Player
               ref={(ref: Player) => { player = ref }}
               data = { musicBeanData[curIndex]}
    ...
    onItemClick(pos){
       curIndex = pos;
       player.notifyUpdate(musicBeanData[curIndex]);
    }

    //下一首
    next(){
      curIndex++;
      if(curIndex >= musicBeanData.length){
        curIndex = 0;
      }
      player.notifyUpdate(musicBeanData[curIndex]);
    }
    ...
    pre(){
        curIndex--;
        if(curIndex < 0){
          curIndex = musicBeanData.length - 1;
        }
      player.notifyUpdate(musicBeanData[curIndex]);
    }
    ...
    //搜索本地歌曲
    search_local_music(){
       MusicSearchModule.getSongList(
        (data) => {
            this.setState({isFirstLoad:false, })
            musicBeanData.splice(0,musicBeanData.length);
            var musicArray = JSON.parse(data);
            //现将数据转成json对象
            for(var obj in musicArray){
                var musicObj = musicArray[obj]
                musicBeanData.push(musicObj);
            }
            this.notifyUpdate(musicBeanData);
       });
    }
